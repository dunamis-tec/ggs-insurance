...
import { LayoutDashboard, Users, FileText, Building2, CreditCard, BookOpen, DollarSign, CheckSquare, LogOut } from 'lucide-react'
...
export default function MainLayout({ session }) {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      <aside style={{ width: '240px', background: '#0C1E3D', display: 'flex', flexDirection: 'column', position: 'fixed', top: 0, left: 0, height: '100vh' }}>
        ...
        <div style={{ padding: '16px', borderTop: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button onClick={handleLogout} style={{ background: 'transparent', color: 'rgba(255,255,255,0.5)', padding: '4px', borderRadius: '4px', cursor: 'pointer' }}><LogOut size={16} /></button>
        </div>
      </aside>
      <main style={{ marginLeft: '240px', flex: 1, padding: '24px', minHeight: '100vh', background: '#F4F6F9' }}>
        <Outlet />
      </main>
    </div>
  )
