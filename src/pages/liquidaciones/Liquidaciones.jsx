...
import { BookOpen, Search, Copy, CheckCircle, Send, Filter, ChevronDown, ChevronUp, ExternalLink } from 'lucide-react'
...
export default function Liquidaciones() {
  ...
  const [loadingReqs, setLoadingReqs] = useState(null)
  ...
  const fetchAll = async () => {
    ...
    const [{ data: reqsData }, { data: asegData }, { data: informesData }] = await Promise.all([
      ...
      supabase.from('informes_enviados').select('*, aseguradoras(nombre, logo_url)').eq('tipo', 'liquidacion').order('created_at', { ascending: false }).limit(100)
    ])
    setReqs(reqsData || [])
  ...
  const fetchInformeReqs = async (informeId) => {
    if (informeReqs[informeId]) return
    setLoadingReqs(informeId)
    const { data } = await supabase.from('requerimientos_pago')
      .select('*, polizas(id, numero_poliza, clientes(id, nombre, apellido))')
      .eq('informe_id', informeId)
      .order('fecha_pago', { ascending: true })
    setInformeReqs(prev => ({ ...prev, [informeId]: data || [] }))
    setLoadingReqs(null)
  }

  const toggleExpandInforme = (id) => {
    if (expandedInforme === id) { setExpandedInforme(null); return }
    setExpandedInforme(id)
    fetchInformeReqs(id)
  }
  ...
  const filteredInformes = informes.filter(inf => {
    ...
    return (inf.aseguradoras?.nombre||'').toLowerCase().includes(q)
      || (inf.fecha_desde||'').includes(q)
      || (inf.fecha_hasta||'').includes(q)
  })
             ...
             filtered.length===0 ? (
              ...
              <div key={r.id} style={{display:'flex',alignItems:'center',padding:'12px 20px',borderBottom:i<filtered.length-1?'1px solid #f1f5f9':'none',background:selectedIds.includes(r.id)?'#eff6ff':'white',cursor:'pointer'}}
                ...
                <div style={{flex:1,minWidth:0}}>
                  <p style={{fontWeight:600,color:'#0C1E3D',fontSize:'13px',textAlign:'left'}}>{r.codigo}</p>
                  <p style={{fontSize:'12px',color:'#64748b',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',textAlign:'left'}}>
                ...
                <p style={{width:'100px',fontSize:'12px',color:'#64748b',textAlign:'left'}}>{r.fecha_pago?new Date(r.fecha_pago+'T12:00:00').toLocaleDateString('es-GT'):'-'}</p>
          ...
          <div style={{background:'white',borderRadius:'12px',padding:'14px 16px',border:'1px solid #e2e8f0',marginBottom:'16px'}}>
            <div style={{position:'relative'}}>
              ...
              <input value={searchHistorial} onChange={e=>setSearchHistorial(e.target.value)}
                placeholder='Buscar por aseguradora, fecha...'
            ...
            {loading ? <p style={{padding:'24px',color:'#64748b'}}>Cargando...</p> :
             filteredInformes.length===0 ? (
              <div style={{padding:'48px',textAlign:'center'}}>
                ...
                <p style={{color:'#94a3b8'}}>{searchHistorial ? 'No se encontraron informes' : 'No hay informes enviados aún'}</p>
              </div>
              ...
              <div key={inf.id} style={{borderBottom:i<filteredInformes.length-1?'1px solid #f1f5f9':'none'}}>
                ...
                <div style={{display:'flex',alignItems:'center',padding:'14px 20px',cursor:'pointer',background:'white'}}
                  ...
                  {expandedInforme===inf.id ? <ChevronUp size={16} color='#64748b'/> : <ChevronDown size={16} color='#64748b'/>}
                </div>

                {/* Detalle expandido */}
                {expandedInforme===inf.id && (
                  <div style={{background:'#f8fafc',borderTop:'1px solid #f1f5f9',padding:'0 0 8px'}}>
                    {loadingReqs===inf.id ? (
                      <p style={{padding:'16px 20px',fontSize:'13px',color:'#64748b'}}>Cargando requerimientos...</p>
                    ) : (informeReqs[inf.id]||[]).length === 0 ? (
                      ...
                      <table style={{width:'100%',borderCollapse:'collapse'}}>
                        ...
                        <tbody>
                          {(informeReqs[inf.id]||[]).map((r,ri)=>(
                            <tr key={r.id}
                              style={{borderBottom:ri<(informeReqs[inf.id]||[]).length-1?'1px solid #f1f5f9':'none',cursor:'pointer'}}
                        ...
                        <tfoot>
                          <tr style={{borderTop:'2px solid #e2e8f0',background:'#f1f5f9'}}>
                            ...
                            <td style={{padding:'8px 20px',fontSize:'13px',fontWeight:700,color:'#0C1E3D'}}>
                              Q {(informeReqs[inf.id]||[]).reduce((s,r)=>s+parseFloat(r.monto||0),0).toLocaleString()}
                            </td>
                          </tr>
                        </tfoot>
                      </table>
